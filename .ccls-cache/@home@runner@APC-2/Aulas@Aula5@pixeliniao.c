...
int main() {
  ...
  for (int lin = 0; lin < 10; lin++) {
    for (int col = 0; col < 10; col++) {
      if (lin == 0 || lin == 9 || col == 0 || col == 9) {
        ...
        pchar4[(lin * 10 * 4) + (col * 4) + 3] = 255; // Alpha (transparência)
        pchar4[(lin * 10 * 4) + (col * 4) + 2] = 127; // B
        pchar4[(lin * 10 * 4) + (col * 4) + 1] = 203;   // G
        pchar4[(lin * 10 * 4) + (col * 4)] = 255;     // R
      }
    }
  }
  // insira o seu código acima
  ...
  for(int x = 0; x < 10; x++){
    for(int y = 0; y < 10; y++){
      if(x<=y){
      pchar5 [x * 10 + y].rgba[0] = 0;
      pchar5 [x * 10 + y].rgba[1] = 0;
      pchar5 [x * 10 + y].rgba[2] = 255;
      pchar5 [x * 10 + y].rgba[3] = 0;
      }
      else {
          pchar5 = imagem5;
      }
